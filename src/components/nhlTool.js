...
export function initComparisonTool(container, { shotData, playerData, goalieData, teamData }, width) {
  ...
  modes.forEach(m => {
    ...
    tab.on("click", function() {
      ...
      root.select(".left-search-label").text(isSkater ? "Left Side Shooter" : isGoalie ? "Left Defending Goalie" : "Left Attacking Team");
      ...
      root.select(".right-search-label").text(isSkater ? "Right Side Shooter" : isGoalie ? "Right Defending Goalie" : "Right Attacking Team");
  ...
  function appendSearchSelector(parentDiv, isLeft) {
    ...
    wrapper.html(`
      <label class="${isLeft ? 'left-search-label' : 'right-search-label'}" style="font-weight: 600; display: block; margin-bottom: 6px; font-size: 13px; color: #475569;">Left Side Shooter</label>
